...
The returned logical vector can be used to subset a vector of the same length.

```{r}
# return the area values > 100000
state.area[state.area > 100000]

# alternatively find the index of the TRUE values using which()
...
is_greater <- state.area > 100000
length(which(is_greater))
sum(is_greater)
```
...
Values in a vector can be also replaced or added by assignment at specific indexes.

```{r}
# What if Colorado was named to Colorodo?
state.name[6] <- "Colorodo"

# what if we wanted to add a 51st state to our vector?
state.name[51] <- "Puero Rico"
```
...
Arithmetic  (e.g `+` `-`, `*`) and comparisons operators (`>`, `<`, `==`) are also vectorized. 
...
2) Knit the Rmarkdown after you make changes, you may need to install the `distill` package: (`install.packages("distill")`), which is used for formatting the rmarkdown. 
