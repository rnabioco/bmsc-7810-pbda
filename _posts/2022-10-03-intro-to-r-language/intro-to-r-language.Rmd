...
It's free, [open-source](https://github.com/wch/r-source), and has been around in it's first public release in 1993. 


## The R ecosystem

When you download R from CRAN, there are a number of packages included in the base installation (e.g. `base`, `stats`, and `datasets`). You can do effective data analysis with only the base installation (e.g. [faster](`https://github.com/matloff/fasteR`). However a key strength of R is the 10,000+ user-developed packages which extend base R functionality. 


```{r, echo = FALSE, fig.cap = "Major R package repositories and functions used to install packages."}
knitr::include_graphics("img/r-ecosystem.jpg")
```
...
Some very good ideas and examples are provided here:
...
1) Use comments to remind yourself what the code does
...
5) Split up complicated operations or long function calls into multiple lines. In general you can add a newline after a comma. Indenting the code can also help with readability.
...
knitr::include_graphics("img/dataStructures.png")
```
...
See `Help > Cheatsheets` for very helpful graphical references. Print them out and pin them up next to your screen. The `base R`, `dplyr`, and `ggplot2` cheatsheets are especially useful.
...
```{r, echo = FALSE, fig.cap = "Data analysis workflow."}
knitr::include_graphics("img/tidy-data-model.png")
```

In this course we will introduce how to conduct each of these key steps in data analysis in R. 


## Exercises

See problem_set1.Rmd (problems 1 - X) for exercises related to this content. 
