...
Returning to our state data, say we wanted to find states from two different geographic regions, such as the south and west. How might we approach this?
...
Alternatively we can use the `!` operator with `%in%`:
...
Whebn printing the `state.name` object you may have noticed the `Levels: Northeast South North Central West`. What is this?
...
Internally they are represented as integers, which levels (names) associated with each integer value.
...
For example, what if we wanted to determine a state name corresponding to a 
state abbreviation?
...
With names, now we query the vector by abbreviations, which will then return the state names. 
...
A `list` is similar to a vector however it can contain elements from different classes or types. Each element can have a different length or type and can even be a list to generate a nested list of lists. 
...
Individual columns can be accessed using the `$` symbol and treated like regular vectors.
...
Just like with vectors, we can also subset data.frames using logical vectors, positions, and names if they're are column and rownames. 
...
This data.frame has row names, which are names that denote individual rows and column names that indicate columns. We can subset columns and rows using these names.

```{r}
rownames(mtcars)[1:5]
colnames(mtcars)[1:5]

mtcars["Duster 360", c("cyl", "hp")]
```
...
In contrast, a R command may return a message or warning, both of which will not end the execution, but are providing some information about the command being run. Warnings generally should not be ignored as they often are pointing to issues you need to address.
...
Often in your analysis code it is useful to throw an error if something strange happens. `stopifnot()` is a useful command to do this. 
