...
We can also use `as.matrix` on an exisiting dataframe
...
Matricies can only have values of one type --> integer, boolean, character, while a dataframe can be a mix of types:
...
But Matricies can take any type of input
...
We can also perform these operations on two matricies
...
**Exercise**
Multiply, subtract, and divide the two matricies `M1` and `M2`
...
First let's download the file I made

```{r}
# This downloads the file and saves it anywhere you want - let's just save
# it in the same direcotory as this script for simplicity
download.file("https://raw.githubusercontent.com/kwells4/pdba_files/main/files/boy_name_counts.csv",
              destfile = "boy_name_counts.csv")
```

We can now read in the data and convert it to a matrix

```{r}
names_mat <- read.csv("boy_name_counts.csv", row.names = 1) %>%
  as.matrix()
...
```{r}
min(names_mat[, "Alabama_number_of_males"])
```
...
```{r}
mean(names_mat[ , "Alabama_number_of_males"])
```

```{r}
median(names_mat[ , "Alabama_number_of_males"])
```

```{r}
sd(names_mat[ , "Alabama_number_of_males"])
```
...
Note how after the transformation, all items in the original df are now characters and we no longer have a dataframe.

Now let's try this transformation on the names matrix we've been working with

```{r}
transformed_mat <- t(names_mat)

transformed_mat[1:3,1:3]
```

Note how the columns are now names and the rows are now states.

Remember the note above where we could only divide by the `rowSums`? Now we can use this transposition to figure out the percent of children in each (divide the value by the column sum * 100)

```{r}
state_percents <- transformed_mat / rowSums(transformed_mat) * 100
...
Next, we need to `pivot_longer` from `tidyr`. Because all of the columns that we want to shift longer end with "number_of_males" we can use `contains` to find all of these columns

```{r}
names_dat <- pivot_longer(names_dat, cols = contains("number_of_males"),
...
names_dat <- names_mat %>% 
  ...
  pivot_longer(cols = contains("number_of_males"), 
               names_to = "state", values_to = "count")
```
...
normalized_dat <- normalized_mat %>% 
  ...
  rownames_to_column("name") %>% 
  pivot_longer(cols = contains("number_of_males"), 
               names_to = "state", values_to = "normalized_count")

ggplot(normalized_dat, aes(x = name, y = normalized_count,
                              fill = name)) + 
  geom_violin() +
  theme(axis.text.x = element_text(angle = 90,
                                   vjust = 0.5, hjust=1)) # rotate x axis
...
https://bookdown.org/manishpatwal/bookdown-demo/matrices-in-r.html
https://monashbioinformaticsplatform.github.io/r-intro/matrices.html#:~:text=In%20a%20data%20frame%20the,lets%20these%20be%20used%20interchangably.
https://monashbioinformaticsplatform.github.io/r-intro/matrices.html#:~:text=In%20a%20data%20frame%20the,lets%20these%20be%20used%20interchangably.
