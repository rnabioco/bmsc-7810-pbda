...
- Use filter to select for flights to DEN with a delayed departure (`dep_delay`).
...
- Use arrange to rank the data by flight distance (`distance`), rank in ascending order. What flight is the shortest distance?
...
select(flights, distance, hour, everything())
```
...
str_c(some_words, " words words words")
