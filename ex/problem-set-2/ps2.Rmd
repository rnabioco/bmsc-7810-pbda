...
ggplot(mtcars, aes(x = cyl)) +
  ...
  labs(title = "Number of cars with different numbers of cylinders",
...
ggplot(data = iris, aes(x = Species, y = Petal.Length)) +
  geom_jitter() +
  annotate(geom = "point", color = "red", y = mean(filter(iris, Species == "setosa")$Petal.Length), x="setosa" ) +
  annotate(geom = "point", color = "red", y = mean(filter(iris, Species == "versicolor")$Petal.Length), x="versicolor" ) +
  annotate(geom = "point", color = "red", y = mean(filter(iris, Species == "virginica")$Petal.Length), x="virginica" )
```
...
```{r}
ggplot(data=diamonds) + 
  geom_histogram(binwidth=0.01, aes(x=price/carat)) + 
 + facet_wrap(~cut) + scale_x_log10()
```
...
```{r fig.height = 10}
ggplot(data = mpg) +
  geom_boxplot(mapping = aes(x= model, y = hwy)) +
