...
```{r, q1}
library(tidyverse)
dat <- read_csv("https://github.com/rnabioco/bmsc-7810-pbda/raw/main/data/class3/dmel_peptides_lifecycle.csv.gz")
dat
```
...
```{r, q4}
length(unique(dat$Proteins))
```

### Question 5 - Filter the data

The authors noted that they detected a large number of yeast proteins which likely derived from the yeast that are a food source for drosophila. The drosophila proteins have transcript IDs (`Proteins` column) that start with `FBtr` and the yeast start with `Y` or `Q0`. 
...
```{r, q6}
a <- pivot_longer(dat, cols = -c(Sequence, Proteins)) %>% 
  separate(name, c("stage", "intensity"))
...
```{r, q8}
b %>% filter(stage == "f") %>% arrange(desc(mean_intensity))
```
